...
`flexible.js`默认是将屏幕划分成 10 等份，手动改成 24 份。那么 `1rem = 1920px / 24 = 80px`

把 vscode 中的 cssrem 插件 `Root Font Size` 设置为 80px 
...
.map {
  ...
  @keyframes rotate {
    from {
      transform: translate(-50%, -50%) rotate(0deg);    <!-- 注意要加上 translate 的  -->
    }
    to {
